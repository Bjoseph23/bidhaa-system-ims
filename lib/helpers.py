        ...
        try:
            ...
            if product_id:
                if not Product.find_by_id(product_id):
                    print(f'Product with id {product_id} not found.')
                    return
                order_history.product = Product.find_by_id(product_id)
...
def create_order_details():
    ...
    if not Supplier.find_by_id(supplier_id):
        print(f'Supplier with id {supplier_id} not found.')
        return
    quantity = int(input("Enter the order quantity: "))
    order_date = input("Enter the order date (YYYY-MM-DD): ")
    try:
        order_detail = OrderDetails.create(Product.find_by_id(product_id), Supplier.find_by_id(supplier_id), quantity, order_date)
        ...
        try:
            ...
            if product_id:
                if not Product.find_by_id(product_id):
                    print(f'Product with id {product_id} not found.')
                    return
                order_detail.product = Product.find_by_id(product_id)
            supplier_id = int(input("Enter the supplier's new id (optional): "))
            if supplier_id:
                if not Supplier.find_by_id(supplier_id):
                    print(f'Supplier with id {supplier_id} not found.')
                    return
                order_detail.supplier = Supplier.find_by_id(supplier_id)
            quantity = int(input("Enter the order quantity (optional): "))
            if quantity > 0:
